...
<?php $this->end() ?> 
<div class="work-copy">
    <?= $this->Form->create($mentor, ['class' => 'form-horizontal']) ?>
    ...
    <div class="form-group">
        ...
        <div class="col-md-8">
            <div style="width:60px;height:60px;padding:0px;"  class="img-thumbnail input-img img-circle"  single>
                <img style="width:60px;height:60px" class="img-circle"  alt="" src="<?= $mentor->avatar ?>"/>
            </div>
            <div style="color:red">(tips:头像尺寸不要太大，正方形最佳)</div>
            <input name="avatar" value="<?= $mentor->avatar ?>"  type="hidden"/>
    ...
    <div class="form-group">
        ...
        <div class="col-md-8">
            <?php
            echo $this->Form->input('introduce', ['label' => false, 'class' => 'form-control']);
            ?>
        </div>
    </div>
    <div class="form-group">
...
<script>
    $(function () {
        initJqupload('avatar', '/wpadmin/util/doUpload?dir=/mentor/avatar', 'jpg,png,gif,jpeg'); //初始化图片上传
                    ...
                    if (typeof res === 'object') {
                        if (res.status) {
                            layer.alert(res.msg, function () {
                                window.location.href = '/admin/mentor/index';
                            });
                        } else {
