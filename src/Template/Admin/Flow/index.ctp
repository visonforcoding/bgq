...
<script>
                $(function () {
                    ...
                    $("#list").jqGrid({
                        url: "/admin/flow/getDataList",
                        datatype: "json",
                        mtype: "POST",
                        colNames:
                                ['用户','公司','职位',  '交易类型', '是否收入', '交易金额', '交易前金额', '交易后金额','支付方式',  '备注', '创建时间',  '操作'],
                        colModel: [
                            {name: 'buyer.truename', editable: true, align: 'center'},
                            {name: 'buyer.company', editable: true, align: 'center'},
                            {name: 'buyer.position', editable: true, align: 'center'},
                            {name: 'type_msg', editable: true, align: 'center'},
                            {name: 'income', editable: true, align: 'center',formatter:function(cell,opt,obj){
                                    if(cell==1){
                                        return '收入';
                                    }else{
                                       return '支出';
                            ...
                            {name: 'pre_amount', editable: true, align: 'center'},
                            {name: 'after_amount', editable: true, align: 'center'},
                            {name: 'paytype', editable: true, align: 'center',formatter:function(cell,opt,obj){
                                    switch(cell){
                                        case 1:
                                        return '微信';
                                        case  2:
...
//                    response += '<a title="编辑" href="/admin/flow/edit/' + rowObject.id + '" class="grid-btn "><i class="icon icon-pencil"></i> </a>';
                    return response;
                }
                ...
                function doExport() {
                    ...
                    $("body").append("<iframe src='/admin/flow/exportExcel?" + searchQueryStr + "' style='display: none;' ></iframe>");
                }

                function doView(id) {
                    //查看明细
                    url = '/admin/flow/view/' + id;
                    layer.open({
                        type: 2,
                        title: '查看详情',
                        shadeClose: true,
