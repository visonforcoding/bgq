    ...
    <form id="table-bar-form">
        <div class="table-bar form-inline">
            <div class="form-group">
                ...
                <input type="text" name="keywords" class="form-control" id="keywords" placeholder="输入名片夹主人名字">
            </div>
            <div class='form-group'>
                <label for="keywords">是否回赠</label>
                <select name='reorder' class='form-control'>
                    <option value="">请选择</option>
                    <option value="1">未回赠</option>
                    <option value="2">已回赠</option>
                </select>
            </div>
            <div class="form-group">
...
<script>
                $(function () {
                    ...
                    $("#list").jqGrid({
                        url: "/admin/cardbox/getDataList",
                        datatype: "json",
                        mtype: "POST",
                        colNames:['名片夹主人', '递名片用户', '是否回赠', '创建时间', '操作'],
                        colModel: [
                            {name: 'my_card_case.truename', editable: true, align: 'center'},
                            {name: 'other_card.truename', editable: true, align: 'center'},
                            {name: 'resend', editable: true, align: 'center', formatter: resendFormatter},
                ...
                function resendFormatter(cellvalue, options, rowObject){
                    if(rowObject.resend == 1)
                    {
                        response = '未回赠';
                    }
                    else if(rowObject.resend == 2)
                    {
                        response = '已回赠';
                    }
                    return response;
                ...
                function doExport() {
                    ...
                    $("body").append("<iframe src='/admin/cardbox/exportExcel?" + searchQueryStr + "' style='display: none;' ></iframe>");
                }

                function doView(id) {
                    //查看明细
                    url = '/admin/cardbox/view/' + id;
                    layer.open({
                        type: 2,
                        title: '查看详情',
                        shadeClose: true,
