...
<div class="col-xs-12">
    <form id="table-bar-form">
        <div class="table-bar form-inline">
            <a href="/admin/recharge-gift/add" class="btn btn-small btn-warning">
...
<script>
                $(function () {
                    ...
                    $("#list").jqGrid({
                        url: "/admin/recharge-gift/getDataList",
                        datatype: "json",
                        mtype: "POST",
                        colNames:
                                ['充值金额', '充值赠送金额', '操作'],
                        colModel: [
                            {name: 'recharge_money', editable: true, align: 'center'},
                            {name: 'gift', editable: true, align: 'center'},
...
//                    response += '<a title="查看" onClick="doView(' + rowObject.id + ');" data-id="' + rowObject.id + '" class="grid-btn "><i class="icon icon-eye-open"></i> </a>';
                    response += '<a title="编辑" href="/admin/recharge-gift/edit/' + rowObject.id + '" class="grid-btn "><i class="icon icon-pencil"></i> </a>';
                    return response;
                }
                ...
                function delRecord(id) {
                    layer.confirm('确定删除？', {
                        ...
                        $.ajax({
                            ...
                            data: {id: id},
                            dataType: 'json',
                            url: '/admin/recharge-gift/delete',
                ...
                function doExport() {
                    ...
                    $("body").append("<iframe src='/admin/recharge-gift/exportExcel?" + searchQueryStr + "' style='display: none;' ></iframe>");
                }

                function doView(id) {
                    //查看明细
                    url = '/admin/recharge-gift/view/' + id;
                    layer.open({
                        type: 2,
                        title: '查看详情',
                        shadeClose: true,
