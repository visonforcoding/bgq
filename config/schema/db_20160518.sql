...
-- 正在导出表  binggq.need 的数据：~1 rows (大约)
DELETE FROM `need`;
...
CREATE TABLE IF NOT EXISTS `smsmsg` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=27 DEFAULT CHARSET=utf8 COMMENT='短信记录';

-- 正在导出表  binggq.smsmsg 的数据：~18 rows (大约)
DELETE FROM `smsmsg`;
...
CREATE TABLE IF NOT EXISTS `usermsg` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8 COMMENT='用户消息';

-- 正在导出表  binggq.usermsg 的数据：~2 rows (大约)
DELETE FROM `usermsg`;
